 ...
 *
 * @author ATG
 * @version $Id: //hosting-blueprint/B2CBlueprint/version/10.2/EStore/src/atg/projects/store/order/StoreShippingGroupManager.java#3 $
 * @updated $DateTime: 2013/02/05 00:41:33 $$Author: jsiddaga $
 */
public class StoreShippingGroupManager extends ShippingGroupManager {
  
  /** Class version string. */
  public static final String CLASS_VERSION = "$Id: //hosting-blueprint/B2CBlueprint/version/10.2/EStore/src/atg/projects/store/order/StoreShippingGroupManager.java#3 $$Change: 788278 $";
   ...
   * 
   * @return true if the number of commerce items in the order (except gift wrap)
   *         is more than one, otherwise return false.
   */
  public boolean isMultipleNonGiftWrapItems(Order pOrder) {
    long nonGiftWrapItemsCount = 0;
    
    if(((StoreOrderImpl)pOrder).getContainsGiftWrap()) {
      nonGiftWrapItemsCount = pOrder.getTotalCommerceItemCount() - 1;
    } 
    else {
      nonGiftWrapItemsCount = pOrder.getTotalCommerceItemCount();
    }
    
    return nonGiftWrapItemsCount > 1;
  }
}
