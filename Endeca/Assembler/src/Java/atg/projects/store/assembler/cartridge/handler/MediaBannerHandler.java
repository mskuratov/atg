...
public class MediaBannerHandler extends NavigationCartridgeHandler {

  //-------------------------------------
  /** Class version string */

  public static String CLASS_VERSION = "$Id: //hosting-blueprint/B2CBlueprint/version/10.2/Endeca/Assembler/src/atg/projects/store/assembler/cartridge/handler/MediaBannerHandler.java#2 $$Change: 791340 $";
  ...
  public NavigationStateBuilder getNavigationStateBuilder() {
    return navigationStateBuilder;
  }

  public void setNavigationStateBuilder(
      NavigationStateBuilder navigationStateBuilder) {
  ...
  public ContentItem process(ContentItem config)
      throws CartridgeHandlerException {
    MediaBanner mediaBanner = new MediaBanner(config);

    try {
      LinkBuilder linkBuilder = ((MediaBannerConfig) config).getLink();
      if (linkBuilder != null)
        mediaBanner.setLink(linkBuilder.createAction(navigationStateBuilder, contentSource, getActionPathProvider()));
  ...
  protected MediaBannerConfig wrapConfig(ContentItem pContentItem) {
    return new MediaBannerConfig(pContentItem);
  }
}